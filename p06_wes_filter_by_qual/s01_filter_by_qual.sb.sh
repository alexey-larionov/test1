...
## s01_filter_by_qual.sb.sh
## Wes library: filtering raw vcf by qual
## SLURM submission script
## Alexey Larionov, 14Oct2015

## Name of the job:
#SBATCH -J filter_by_qual
...
qual_threshold="${3}"
scripts_folder="${4}"
filtered_vcf_folder="${5}"
log="${6}"

## Start resources monitoring on the background
"${scripts_folder}/a01_resources_monitor.sh" 1 "${filtered_vcf_folder}/${raw_vcf%_raw.vcf}_qual${qual_threshold}.res" &
resources_monitor_pid="${!}" # Take note of the resources monitor PID
echo "Started resources monitoring: $(date +%d%b%Y_%H:%M:%S)"
echo ""

## Do the job
"${scripts_folder}/s01_filter_by_qual.sh" \
         "${job_file}" \
         "${scripts_folder}" &>> "${log}"
