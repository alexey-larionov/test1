...
rsync -thrqe "ssh -x" "${data_server}:${project_location}/${project}/${source_vcf_folder}/${source_vcf}" "${annotated_vcf_folder}/${source_vcf%.vcf}_source_files/"
rsync -thrqe "ssh -x" "${data_server}:${project_location}/${project}/${source_vcf_folder}/${source_vcf}.idx" "${annotated_vcf_folder}/${source_vcf%.vcf}_source_files/"
...
# Progress report
echo "VEP script: adding vep annotations to a vcf file"
...
perl "${vep_script}" \
  ...
  --fields "${vcf_vep_fields}" &>> "${vep2vcf_log}"

# Progress report
echo "Completed: $(date +%d%b%Y_%H:%M:%S)"
echo ""
echo "VEP script: writing vep annotations to a text file"
...
perl "${vep_script}" \
  ...
  --gmaf --maf_1kg --maf_esp \
  --force_overwrite --fork 12 --no_progress \
  --fields "${txt_vep_fields}" &>> "${vep2txt_log}"
