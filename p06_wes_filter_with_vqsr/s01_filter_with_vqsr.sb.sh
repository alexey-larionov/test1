...
"${scripts_folder}/a01_resources_monitor.sh" 1 "${vqsr_vcf_folder}/${source_vcf%_raw.vcf}_vqsr.res" &
resources_monitor_pid="${!}" # Take note of the resources monitor PID
echo "Started resources monitoring: $(date +%d%b%Y_%H:%M:%S)"
echo ""

## Do the job
"${scripts_folder}/s01_filter_with_vqsr.sh" \
         "${job_file}" \
         "${scripts_folder}" &>> "${log}"
